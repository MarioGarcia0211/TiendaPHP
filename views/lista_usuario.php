...
<body>
    ...
    $insert = mysqli_query($conection, "INSERT INTO log_navegacion (email, descripcion) VALUES ('$email', 'Este usuario esta navegando en la lista de los usuarios')");
    ?>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-fixed table-striped">
                            ...
                            <tbody class="text-center">
                                ...
                                if($_SESSION['rol'] == '1'){
                                $query = mysqli_query($conection, "SELECT usuarios.idUsuario, usuarios.nombre, usuarios.apellido, usuarios.email, usuarios.idRol, roles.rol FROM usuarios INNER JOIN roles ON usuarios.idRol = roles.idRol WHERE estado = 1 ORDER BY `usuarios`.`idUsuario` ASC LIMIT $desde,$por_pagina");
                                }else{
                                    $query = mysqli_query($conection, "SELECT usuarios.idUsuario, usuarios.nombre, usuarios.apellido, usuarios.email, usuarios.idRol, roles.rol FROM usuarios INNER JOIN roles ON usuarios.idRol = roles.idRol WHERE estado = 1 AND usuarios.idRol != 1 ORDER BY `usuarios`.`idUsuario` ASC LIMIT $desde,$por_pagina");
                                }
